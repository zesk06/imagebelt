...
def main():
    ...
    parser.add_argument('--verbose', '-v',
                        action='store_true',
                        help='goes to verbose', default=False)
    ...
    metadata_parser = subparsers.add_parser('db',
                                            help='read metadata from JSON')
...
def json_to_image(args):
    ...
    for media in json_obj['media']:
        ...
        tags = media['tags']
        comments = ['%s:%s' % (com['author'], com['comment'])
                    for com in media['comments']]
        lat_lon = None
        if 'geoInfo' in media and media['geoInfo']:
            latitude = media['geoInfo']['latitude_']
            longitude = media['geoInfo']['longitude_']
            lat_lon = (latitude, longitude)
        exiftool(img_path, title=title, description=description,
                 tags=tags, comments=comments, lat_lon=lat_lon)


def exiftool(image_path, title, description=None,
             tags=None,
             comments=None, lat_lon=None):
    ...
    if tags:
        for tag in [tag.strip() for tag in tags]:
            ...
            cmd += ['-Keywords="%s"' % tag]
    if lat_lon:
        cmd += ['-GPSLatitude="%s"' % lat_lon[0],
                '-GPSLongitude="%s"' % lat_lon[1]]
    # to change date:
    # -alldates="20100101 080000" -filemodifydate="20100101 080000"
    logging.debug('calling %s', cmd)
    subprocess.call(cmd, cwd=image_path.parent,
